...
fyne-cross linux --arch arm64 -app-id app.owlcms.obsreplays -app-version $VERSION_TAG -app-build ${MAPPED_RELEASE}${PRE_RELEASE} -icon Icon.png ./cmd/obsreplays 

# Package the app for Windows
fyne-cross windows --app-id app.owlcms.obsreplays -app-version $VERSION_TAG -app-build ${MAPPED_RELEASE}${PRE_RELEASE} -icon Icon.png ./cmd/obsreplays
...
gh release create $TAG -F ReleaseNotes.md $PRERELEASE_FLAG --title "owlcms jury obsreplays $TAG"

# Upload the executables
gh release upload $TAG fyne-cross/bin/linux-arm64/obsreplays fyne-cross/bin/windows-amd64/obsreplays.exe
